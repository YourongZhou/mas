...
{skill_section}
【代码生成硬性约束】
1. 路径规范：代码在 Docker 内运行。默认数据目录为 {docker_data_path}，产出写入 {docker_output_path}。若用户提示中给出了“已验证的容器内输入路径映射”，必须优先使用那些确切路径。针对纯计算任务无文件 IO 则无需强行读盘。
...
6. 环境事实优先：若提示中给出了“当前 Docker 环境完整包版本清单”或类似环境版本文件，那就是当前运行环境的权威事实。你必须基于该清单选择兼容 API，不要凭记忆猜测包版本或函数是否存在。
7. 绘图准则：仅当明确需要绘图任务时才引入 matplotlib（必须选用 Agg 后端），以 plt.savefig 保存至 {docker_output_path} 下即可，严禁 show=True。纯数值任务免引入绘图库。
8. 文件校验：若用户提供了输入文件列表或容器内映射，必须使用其确切路径，禁止臆造其它文件名或其它目录。
9. 读表容错：遇到读取 .panel / 样本列表等文本时，要求使用 pandas.read_csv(..., sep=r"\\\\s+", comment='#', header=None) 此类容错方法，绝对禁止假定表格中存在名为 ID 的列。
...
你的工作任务：先使用 `list_files` 或 `glob_search` 浏览该技能对应的文件目录结构，必须重点关注 `{wf_host_path}` 等相关路径；随后请从目录中找出与接下来任务最相关的极少数核心 `scripts` / `references` 脚本或示例，调用 `read_file` 查看 1-5 个最相关文件内容。必要时可以用 `grep_text` 搜索函数名、关键参数或文档片段，用 `file_exists` 做路径确认。切忌盲目读取所有脚本，完整的 SKILL.md 信息将在此调研结束后自动添加给实际编写代码的模型！

【要求】
- 必须且仅允许使用 Tool Calling （如 `list_files` / `glob_search` / `read_file` / `grep_text` / `file_exists` / `exec_command`）发起工具调用请求。
- 优先使用 `list_files`、`glob_search`、`read_file`、`grep_text` 等只读工具；只有在明确需要查看受限命令输出时才使用 `exec_command`。
